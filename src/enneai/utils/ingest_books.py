...
from enneai.ai.rag.ingest import BookMetadata, ingest_one
...
def discover_books_dirs(data_dir: Path) -> list[Path]:
    ...
    for typology_dir in sorted(data_dir.iterdir()):
        if not typology_dir.is_dir():
            continue

        books_dir = typology_dir / "books"

        if books_dir.is_dir():
            result.append(books_dir)
...
async def ingest_books_dir(
    ...
    *,
    ...
    if not metadata_path.exists():
        logger.warning(
            "Skipping %s: metadata.json is not found",
            books_dir,
        )
        return 0
    ...
    for filename, book_data in metadata.items():
        ...
        if not isinstance(book_data, dict):
            logger.warning(
                "Skipping %s: metadata should be a JSON object",
                filename,
            )
            continue
        ...
        if not book_path.exists():
            logger.warning(
                "File from metadata.json not found: %s",
                book_path,
            )
            continue
        ...
        book_metadata = metadata_from_dict(book_data)

        logger.info(
            "→ %s / %s",
            books_dir.parent.name,
            filename,
        )
...
async def ingest_all(
    *,
    ...
    if not books_dirs:
        logger.warning(
            "Skipping %s: no books directories found",
            data_dir,
        )
        return
